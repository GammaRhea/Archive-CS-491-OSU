...
const char *WINDOWTITLE = { "Particle System -- Conner Rhea" };
...
struct fire
{
	...
	float vx_i, vy_i, vz_i;	// init velocity
...
Display( void )
{
	...
	double p_size = (double)Ranf(3.f, 7.f);
	glPointSize( p_size );
...
InitGraphics( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		float vel = Ranf(VELMIN, VELMAX);
		float theta = Ranf(THETAMIN, THETAMAX);
		float th_pi = Ranf(0.f, 2.f * M_PI);
		double x_rot = (cos(th_pi) + sin(th_pi));
		double z_rot = (cos(th_pi) - sin(th_pi));

		p->vx_i = (vel * cos(theta));
		p->vx_i *= x_rot;

		p->vy_i = vel * sin(theta);

		p->vz_i = (vel * cos(theta));
		p->vz_i *= z_rot;
		...
		p->vy = p->vy_i;
		p->vz = p->vz_i * Time;
		p->t0 = Ranf(TMIN, TMAX / 1.5f);
		p->t1 = Ranf(TMIN / 1.5f, TMAX);
		// p->rad = 0.;
		p->r = Ranf(0.3f, 0.9f);
		p->g = Ranf(0.f, 0.5f);
		p->b = Ranf(0.3f, 0.9f);
